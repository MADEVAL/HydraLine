/// Island dispatcher (pretty, branded).
///
/// Watches every `<hydraline-island>`, fires hydration directives, loads the
/// Flutter engine once and mounts one FlutterView per island via
/// `app.addView()` with `{ islandId, state }` initialData - consumed on the
/// Dart side by `IslandViewRegistry`/`IslandHost`.
///
/// Kept byte-identical to `web/hydraline-dispatcher.js` (verified by test).
library;
